...
_MK_PRIVATE int mk__demuxer_thread(void* data)
{
    MKThreadContext* context = data;
    MKMessageQueue*  msg_q   = context->msg_q;
    MKMessageData    msg;

    while (1) {
        if (*context->is_aborted == 1) {
            break;
        }

        if (mk_message_queue_has_messages(msg_q)) {
            mk_message_queue_get_message(msg_q, &msg);
            if (msg.kind == MK_MESSAGEKIND_DEMUX) {
                printf("START_DEMUX %d\n", msg.data.handle.slot_id);
            }
        }
    }
...
void mk_thread_manager_init(
    MKThreadManager* manager, MKDecoderPool* decoder_p, MKMessageQueue* msg_q
)
{
    MK_ASSERT(manager);
    MK_ASSERT(decoder_p);
    MK_ASSERT(mk_cond_init(&manager->demuxer_continue_signal) >= 0);
    MK_ASSERT(mk_cond_init(&manager->video_continue_signal) >= 0);

    manager->context.decoder_p  = decoder_p;
    manager->context.msg_q      = msg_q;
    manager->context.is_aborted = &manager->is_aborted;

    MKThread* demuxer_thread = &manager->demuxer_thread;
    demuxer_thread->name     = "demuxer_thread";
    demuxer_thread->fn       = mk__demuxer_thread;
    demuxer_thread->userdata = &manager->context;

    manager->is_aborted = 1;
}

void mk_thread_manager_uninit(MKThreadManager* manager)
{
    MK_ASSERT(manager);
    mk_cond_destroy(&manager->demuxer_continue_signal);
    mk_cond_destroy(&manager->video_continue_signal);

    MKThread* demuxer_thread = &manager->demuxer_thread;
    if (demuxer_thread) {
        demuxer_thread->userdata = NULL;
        demuxer_thread->name     = NULL;
        demuxer_thread->fn       = NULL;
    }

    manager->is_aborted = 1;
}

int mk_thread_manager_start(MKThreadManager* manager)
{
    MK_ASSERT(manager);

    manager->is_aborted = 0;

    MKThread* demuxer_thread = &manager->demuxer_thread;
    if (mk_thread_init(demuxer_thread) < 0) {
        return -1;
    }
    return 0;
}

void mk_thread_manager_stop(MKThreadManager* manager)
{
    MK_ASSERT(manager);
    manager->is_aborted      = 1;
    MKThread* demuxer_thread = &manager->demuxer_thread;
...
//         status = mk_init_frame_queue(&video->frame_q, &video->packet_q, 16,
//         1); if (status != 0) {
...
// cleanup_frame_q:
//     mk_destroy_frame_queue(&decoder->video.frame_q);
...
//             status
//                 = mk_packet_queue_get(packet_queue, decoder->packet, 1,
//                 NULL);
