...
int main(void)
{
    ...
    mk_media_decoder_start(&decoder);
    sleep(2);
    mk_media_decoder_read_frame2(&decoder, &target);
    mk_image_data_save_ppm(&target, "tmp/image2.ppm");

    sleep(2);
    mk_media_decoder_read_frame2(&decoder, &target);
    mk_image_data_save_ppm(&target, "tmp/image3.ppm");

    mk_media_decoder_stop(&decoder);
